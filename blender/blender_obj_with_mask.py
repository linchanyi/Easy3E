...
def normalize_scene(shrink =1.0) -> Tuple[float, Vector]:
    ...
    print(scene.matrix_world.translation)
...
def render_and_save(view_id, object_uid, len_val, azimuth, elevation, distance, ortho=False):
    # print(view_id)
    # render the image
    render_path = os.path.join(args.output_dir, 'mask_image', f"{view_id:03d}.png")
...
#     bproc.init()
    ...
    # main_light_data.specular_factor = 0.4
...
#     fill_light.rotation_euler = (-0.05, 0.0, 0.0)
    ...
    # load mask object
    init_objects = set(bpy.context.scene.objects)
    load_object(mask_file)
    mask_objects = set(bpy.context.scene.objects) - init_objects
    apply_norm(mask_objects, scale, offset)
    ...
    for i in range(args.num_images):
        ...
        render_and_save(i , object_uid, -1, azimuth, 0, distance, ortho=True)
    

def img2mask(imgs_path, masks_path, threshold=5):
    for img_id in os.listdir(imgs_path):
        ...
        transparent_mask = (a < 255)
        pure_black_mask = (b <= threshold) & (g <= threshold) & (r <= threshold) & (a == 255)
        final_black_area = transparent_mask | pure_black_mask
        mask_img[final_black_area] = 0
        cv2.imwrite(mask_path, mask_img)

if __name__ == "__main__":
    start_i = time.time()
    local_path = args.object_path
    save_images(local_path, args.mask_path)
    end_i = time.time()
    print("Finished", local_path, "in", end_i - start_i, "seconds")
    
    imgs_path = os.path.join(args.output_dir, 'mask_image')
    masks_path = os.path.join(args.output_dir, 'mask')
    os.makedirs(masks_path, exist_ok=True)
    img2mask(imgs_path, masks_path)
    del_folder(os.path.join(args.output_dir, "mask_image"))
