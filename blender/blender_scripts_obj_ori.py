...
def scene_bbox(
    ...
    # bbox_min[2] = max(bbox_min[2], 0)  # Ensure that the bottom of the bounding box is at z=0.(for glb models)
...
def save_subject(subject, file_path: str):
    with open(file_path, 'w', encoding='utf-8') as f:
        json.dump(subject, f, indent=4)

def normalize_scene(shrink =1.0) -> Tuple[float, Vector]:
    ...
    base = 1 / max(bbox_max - bbox_min)
    scale = base*float(shrink)
...
def render_and_save(view_id, len_val, azimuth, elevation, distance, ortho=False):
    # print(view_id)
    # render the image
    render_path = os.path.join(args.output_dir, 'image_ori', f"{view_id:03d}.png")
    ...
    if not os.path.exists(os.path.join(args.output_dir, 'image_ori', f"{view_id:03d}.png")):
        bpy.ops.render.render(write_still=True)

    
   
    if os.path.exists(os.path.join(args.output_dir,  'normal_ori', f"{view_id:03d}0001.exr")):
        normal = cv2.imread(os.path.join(args.output_dir, 'normal_ori', f"{view_id:03d}0001.exr"), cv2.IMREAD_UNCHANGED)
        normal_unit16 = (normal * 65535).astype(np.uint16)
        cv2.imwrite(os.path.join(args.output_dir,  'normal_ori', f"{view_id:03d}.png"), normal_unit16)
        os.remove(os.path.join(args.output_dir,  'normal_ori', f"{view_id:03d}0001.exr"))
...
def save_images(object_file: str) -> None:
    ...
    if os.path.exists(os.path.join(args.output_dir, 'camera', '64.npy')): return
...
#     bproc.init()
    ...
    back_node.inputs['Strength'].default_value = 1.3
    
    scale, offset = normalize_scene(shrink = args.shrink)
    np.save(os.path.join(args.output_dir,'meta.npy'), np.asarray([scale, offset[0], offset[1], offset[1]],np.float32))
    ...
    subject_width = 1
    save_subject(args.shrink, os.path.join(args.output_dir, "shrink.json"))
    lens = [35, 50, 85, 105, 135]
    
    normal_file_output.base_path = os.path.join(args.output_dir, 'normal_ori')
    ...
    for i in range(args.num_images):
        ...
        render_and_save(i ,  -1, azimuth, 0, distance, ortho=True)
