...
class SparseStructureFlowModel(nn.Module):
    ...
    def create_attention_mask(self,seq_len, mask_indices, device='cuda'):
        
        attention_mask = torch.zeros(seq_len, seq_len, dtype=torch.bool, device=device)


        mask_token_indices = torch.tensor([idx  for idx in mask_indices], device=device)

        all_indices = torch.arange(0, seq_len, device=device)
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, cond: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        h = patchify(x, self.patch_size) #[1,8,16,16,16]
        h = h.view(*h.shape[:2], -1).permute(0, 2, 1).contiguous() #[1,4096,8]
        if kwargs["use_mask"]:
            mask = kwargs["mask"].view(*kwargs["mask"].shape[:2], -1).permute(0, 2, 1).contiguous()
            bool_mask = (mask.sum(dim=2) > 0.5)
            mask_indices = torch.nonzero(bool_mask)[:,1]
            kwargs["mask"]=mask
            kwargs["mask_indices"] = mask_indices
