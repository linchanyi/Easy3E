...
class FlowMatchingTrainer(BasicTrainer):
    ...
    def training_losses(
        ...
        **kwargs
        ...
        t = self.sample_t(x_B.shape[0]).to(x_B.device).float()
        x_t = self.diffuse(x_B, t, noise=noise)
        cond = self.get_cond(cond, **kwargs)
        
        pred = self.training_models['denoiser'](x_A, t * 1000, cond, **kwargs)
        assert pred.shape == noise.shape == x_0.shape
        target = self.get_v(x_B, noise, t)
        terms = edict()
        ...
        t = self.sample_t(x_A.shape[0]).to(x_A.device).float()
        ...
        assert pred.shape == noise.shape == x_B.shape
        target = self.get_v(x_B, noise, t)
